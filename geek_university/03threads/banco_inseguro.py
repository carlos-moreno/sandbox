...
def criar_contas() -> List[Conta]:
    return [
        Conta(saldo=random.randint(5_000, 15_000)),
        Conta(saldo=random.randint(5_000, 15_000)),
        Conta(saldo=random.randint(5_000, 15_000)),
        Conta(saldo=random.randint(5_000, 15_000)),
        Conta(saldo=random.randint(5_000, 15_000)),
...
def valida_banco(contas: List[Conta], total: int):
    ...
    if atual != total:
        print(
            f"ERROR: Balanço bancário inconsistente. BRL$ {atual:.2f} vs {total:.2f}",
            flush=True,
        )
    else:
        print(f"OK: Balanço bancário consistente. BRL$ {total:.2f}", flush=True)
